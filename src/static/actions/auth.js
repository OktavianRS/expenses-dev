...
import { checkHttpStatus, parseJSON, cutStatusInfo } from '../utils';
import {
...
export function authLoginUser(email, password, redirect = '/dasboard') {
    return (dispatch) => {
        ...
        return fetch(`${SERVER_URL}/api/v1/accounts/login/`, {
            ...
            .then((response) => {
                ...
                dispatch(notifyStart(`hello ${response.user.first_name}`))
...
export function authRegisterUser(email, first_name, last_name, password, redirect = '/') {
    return (dispatch) => {
        dispatch(authLoginUserRequest());
        // const auth = btoa(`${email}:${first_name}:${last_name}:${password}`);
        return axios(`${SERVER_URL}/api/v1/accounts/register/`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
